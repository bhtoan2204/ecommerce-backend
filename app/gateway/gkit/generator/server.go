...
func GenerateInitRoutes(protos []*model.Proto) error {
	...
	template, err := util.ReadFileAsString("gkit/template/routing_init.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}

	paramsRoutingTemplate, err := util.ReadFileAsString("gkit/template/routing_init_params.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}

	funcsRoutingTemplate, err := util.ReadFileAsString("gkit/template/routing_init_funcs.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
