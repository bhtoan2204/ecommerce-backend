...
func generatePostHandler(handler *model.Handler, requestType *model.Message) error {
	...
	if handler.Accept == "multipart/form-data" {
		templatePath = "gkit/template/handler_post_file.tmpl"
	} else if handler.Type == "webhook" {
		templatePath = "gkit/template/handler_post_webhook.tmpl"
	} else {
		templatePath = "gkit/template/handler_post.tmpl"
	}
	...
	// handle params
	paramDecTemplate, err := util.ReadFileAsString("gkit/template/param_declaration.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}

	paramParsingIdTemplate, err := util.ReadFileAsString("gkit/template/param_parsing_id.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
	paramParsingFileTemplate, err := util.ReadFileAsString("gkit/template/param_parsing_file.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
	paramParsingEnumTemplate, err := util.ReadFileAsString("gkit/template/param_parsing_enum.tmpl")
	if err != nil {
...
func generatePutHandler(handler *model.Handler, requestType *model.Message) error {
	template, err := util.ReadFileAsString("gkit/template/handler_put.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
...
func generateGetHandler(handler *model.Handler, requestType *model.Message) error {
	template, err := util.ReadFileAsString("gkit/template/handler_get.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
	...
	paramParsingIntTemplate, err := util.ReadFileAsString("gkit/template/param_parsing_int.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
	paramParsingArrIntTemplate, err := util.ReadFileAsString("gkit/template/param_parsing_arr_int.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
	paramParsingStringTemplate, err := util.ReadFileAsString("gkit/template/param_parsing_string.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
	...
	paramParsingArrStringTemplate, err := util.ReadFileAsString("gkit/template/param_parsing_arr_string.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
	paramParsingBoolTemplate, err := util.ReadFileAsString("gkit/template/param_parsing_bool.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
	paramParsingGGTimestampTemplate, err := util.ReadFileAsString("gkit/template/param_parsing_gg_timestamp.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
...
func GenerateRoutingRegistry(handlers []*model.Handler, handlerFolderPaths []string) error {
	...
	template, err := util.ReadFileAsString("gkit/template/routing_registry.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}

	handlerImportTemplate, err := util.ReadFileAsString("gkit/template/handler_import.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
	...
	routingConfigTemplate, err := util.ReadFileAsString("gkit/template/routing_config.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
