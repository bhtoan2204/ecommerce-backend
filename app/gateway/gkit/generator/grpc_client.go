...
func generateGrpcClient(proto *model.Proto, service *model.Service) error {
	...
	template, err := util.ReadFileAsString("gkit/template/grpc_client.tmpl")
	if err != nil {
	...
	methodConfigTemplate, err := util.ReadFileAsString("gkit/template/grpc_method_entry.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
	...
	apiInvocationTemplate, err := util.ReadFileAsString("gkit/template/grpc_api_invocation.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
...
func GenerateServiceClientRegistry(protos []*model.Proto) error {
	...
	template, err := util.ReadFileAsString("gkit/template/service_client_registry.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}

	clientEntryTemplate, err := util.ReadFileAsString("gkit/template/grpc_service_client_entry.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
